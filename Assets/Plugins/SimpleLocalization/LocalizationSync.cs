...
namespace Assets.SimpleLocalization
{
	...
	public class LocalizationSync : MonoBehaviour
	{
        ...
        //https://docs.google.com/spreadsheets/d/1oeJKamKX2pWJr3JmjeNLd0HOoff_di9xrfGFyqK8KTY/edit#gid={1}
		...
		private IEnumerator SyncCoroutine()
		{
			...
			var downloaders = new Dictionary<WWW, Sheet>();

			foreach (var sheet in Sheets)
			{
				var url = string.Format(UrlPattern, TableId, sheet.Id);
				var downloader = new WWW(url);
				downloaders.Add(downloader, sheet);
			}

			foreach (var kvp in downloaders)
			{
				var downloader = kvp.Key;
				var sheet = kvp.Value;
				...
				{
					Debug.LogError($"Error downloading sheet {sheet.Name}: {downloader.error}");
				}
			}
			
			Debug.Log("<color=green>Localization synced!</color>");
		}
